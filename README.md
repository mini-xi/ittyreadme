...
|애플리케이션을 컨테이너라고 불리는 표준화된 단위로 포장하는 기술. 컨테이너는 애플리케이션과 그 애플리케이션이 필요로 하는 모든 종속성을 포함하여 어디에서나 동일하게 실행될 수 있도록 |여러 대의 서버(노드)에 걸쳐 컨테이너화된 애플리케이션을 배포, 관리, 확장하는데 사용되는 오픈소스 오케스트레이션 플랫폼|
